...
if(isset($_GET['ajax'])) {
	...
	if(!empty($search)) {
		$where = " AND (uname LIKE '%".$search."%' OR nama LIKE '%".$search."%' OR level LIKE '%".$search."%')";
	}
	$sql_limit = " LIMIT ".$offset.",".$limit." ";
	if ( isset($_POST['sort']) && isset($_POST['order']) ) {
		$order_by = " ORDER BY ".$_POST['sort']." ".$_POST['order']." ";
	}
	$sql_tampil = "SELECT id, uname, level, nama, aktif FROM data_pengguna WHERE 1=1 ".$where." ".$order_by." ".$sql_limit."";
	$data_list = $db->select($sql_tampil);
	$sql_total = "SELECT id FROM data_pengguna WHERE 1=1 ".$where." ";
...
if(isset($_GET['hapus'])) {
	$id = $_POST['id'];
	$hapus = $db->delete('data_pengguna', $id);
	if($hapus) {
		echo 'OK';
	}
	exit();
		...
		<div class="box box-primary">
			...
			<div class="box-body">
				...
				<div id="toolbar" class="">
					<a href="<?php echo site_url('?m=pengguna&c=tambah'); ?>" class="btn btn-flat btn-success">
				...
				<table 
					...
					data-toggle="table"
					data-id-field="id"
					data-url="<?php echo site_url('?m=pengguna&ajax=1'); ?>" 
					data-sort-name="uname"
					data-sort-order="asc"
					...
					<thead>
						<tr>
							...
							<th data-field="uname" data-sortable="true" data-valign="middle">Username</th>
							<th data-field="nama" data-sortable="true" data-valign="middle">Nama</th>
							<th data-field="level" data-sortable="true" data-valign="middle">Level</th>
...
<script type="text/javascript">
	...
	$(function() {
		...
		$('#link_konfirmasi_hapus').click(function(event) {
			...
			//var id_hapus = 0;
			$.ajax({
				url: '<?php echo site_url('?m=pengguna&hapus=1'); ?>',
				type: 'POST',
				dataType: 'html',
				data: {id: id_hapus},
			})
	...
	function aksi_ft(value, row, index) {
		var nsi_out = '<a class="btn btn-flat btn-sm btn-primary" href="<?php echo site_url('?m=pengguna&c=ubah&id'); ?>=' + row.id + '" title="Ubah"><i class="fa fa-pencil"></i></a>';
		if(row.id >= 1) {
