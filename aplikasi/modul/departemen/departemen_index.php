...
if(isset($_GET['ajax'])) {
	...
	if(!empty($search)) {
		$where = " AND (nama_departemen LIKE '%".$search."%' OR keterangan LIKE '%".$search."%')";
	}
	$sql_limit = " LIMIT ".$offset.",".$limit." ";
	if ( isset($_POST['sort']) && isset($_POST['order']) ) {
		$order_by = " ORDER BY ".$_POST['sort']." ".$_POST['order']." ";
	}
	$sql_tampil = "SELECT id, nama_departemen, keterangan, aktif FROM m_departemen WHERE 1=1 ".$where." ".$order_by." ".$sql_limit."";
	$data_list = $db->select($sql_tampil);

	$sql_total = "SELECT id FROM m_departemen WHERE 1=1 ".$where." ";
...
if(isset($_GET['hapus'])) {
	$id = $_POST['id'];
	$hapus = $db->delete('m_departemen', $id);
	if($hapus) {
		echo 'OK';
	}
	exit();
		...
		<div class="box box-primary">
			...
			<div class="box-body">
				...
				<div id="toolbar" class="">
					<a href="<?php echo site_url('?m=departemen&c=tambah'); ?>" class="btn btn-flat btn-success">
				...
				<table 
					...
					data-toggle="table"
					data-id-field="id"
					data-url="<?php echo site_url('?m=departemen&ajax=1'); ?>" 
					data-sort-name="nama_departemen"
					data-sort-order="asc"
					...
					<thead>
						<tr>
							...
							<th data-field="nama_departemen" data-sortable="true" data-valign="middle">Nama Departemen</th>
							<th data-field="keterangan" data-sortable="true" data-valign="middle">Keterangan</th>
...
<script type="text/javascript">
	...
	$(function() {
		...
		$('#link_konfirmasi_hapus').click(function(event) {
			...
			//var id_hapus = 0;
			$.ajax({
				url: '<?php echo site_url('?m=departemen&hapus=1'); ?>',
				type: 'POST',
				dataType: 'html',
				data: {id: id_hapus},
			})
	...
	function aksi_ft(value, row, index) {
		var nsi_out = '<a class="btn btn-flat btn-sm btn-primary" href="<?php echo site_url('?m=departemen&c=ubah&id'); ?>=' + row.id + '" title="Ubah"><i class="fa fa-pencil"></i></a>';
		if(row.id >= 1) {
