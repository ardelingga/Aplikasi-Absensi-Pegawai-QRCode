...
if(isset($_GET['ajax'])) {
	...
	if(!empty($search)) {
		$where = " AND (tabel_set_abs.nama_set_abs LIKE '%".$search."%' OR m_departemen.nama_departemen LIKE '%".$search."%' OR m_set_waktu.nama_seting LIKE '%".$search."%' OR m_set_priode.nama_priode LIKE '%".$search."%')";
	}
	$sql_limit = " LIMIT ".$offset.",".$limit." ";
	if ( isset($_POST['sort']) && isset($_POST['order']) ) {
		$order_by = " ORDER BY ".$_POST['sort']." ".$_POST['order']." ";
	}
	$sql_tampil = "SELECT 	tabel_set_abs.id AS id, 
							tabel_set_abs.nama_set_abs AS nama_absen, 
							m_departemen.nama_departemen AS nama_departemen, 
							m_set_waktu.nama_seting AS nama_waktu, 
							m_set_priode.nama_priode AS nama_priode 
					FROM tabel_set_abs 
					LEFT JOIN m_departemen ON m_departemen.id = tabel_set_abs.id_departemen
	...
	$sql_total = "SELECT tabel_set_abs.id AS id FROM tabel_set_abs 
					LEFT JOIN m_departemen ON m_departemen.id = tabel_set_abs.id_departemen
					LEFT JOIN m_set_waktu ON m_set_waktu.id = tabel_set_abs.id_waktu
					LEFT JOIN m_set_priode ON m_set_priode.id = tabel_set_abs.id_priode
					WHERE 1=1 ".$where." ";
...
if(isset($_GET['hapus'])) {
	$id = $_POST['id'];
	$hapus = $db->delete('tabel_set_abs', $id);
	if($hapus) {
		echo 'OK';
	}
	exit();
		...
		<div class="box box-primary">
			...
			<div class="box-body">
				...
				<div id="toolbar" class="">
					<a href="<?php echo site_url('?m=absen&c=tambah'); ?>" class="btn btn-flat btn-success">
				...
				<table 
					...
					data-toggle="table"
					data-id-field="id"
					data-url="<?php echo site_url('?m=absen&ajax=1'); ?>" 
					data-sort-name="nama_set_abs"
					data-sort-order="asc"
					...
					<thead>
						<tr>
							...
							<th data-field="nama_absen" data-sortable="true" data-valign="middle">Nama Seting</th>
							<th data-field="nama_departemen" data-sortable="true" data-valign="middle">Departemen</th>
							<th data-field="nama_waktu" data-sortable="true" data-valign="middle">Waktu</th>
							<th data-field="nama_priode" data-sortable="true" data-valign="middle">Periode</th>
...
<script type="text/javascript">
	...
	$(function() {
		...
		$('#link_konfirmasi_hapus').click(function(event) {
			...
			//var id_hapus = 0;
			$.ajax({
				url: '<?php echo site_url('?m=absen&hapus=1'); ?>',
				type: 'POST',
				dataType: 'html',
				data: {id: id_hapus},
			})
	...
	function aksi_ft(value, row, index) {
		var nsi_out = '<a class="btn btn-flat btn-sm btn-primary" href="<?php echo site_url('?m=absen&c=ubah&id'); ?>=' + row.id + '" title="Ubah"><i class="fa fa-pencil"></i></a>';
		if(row.id >= 1) {
