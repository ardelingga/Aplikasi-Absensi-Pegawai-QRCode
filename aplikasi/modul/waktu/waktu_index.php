...
if(isset($_GET['ajax'])) {
	...
	if(!empty($search)) {
		$where = " AND (nama_seting LIKE '%".$search."%')";
	}
	$sql_limit = " LIMIT ".$offset.",".$limit." ";
	if ( isset($_POST['sort']) && isset($_POST['order']) ) {
		$order_by = " ORDER BY ".$_POST['sort']." ".$_POST['order']." ";
	}
	$sql_tampil = "SELECT id, nama_seting,
						LEFT(jam_mulai_abs, 5) AS jam_mulai_abs,
						LEFT(jam_masuk, 5) AS jam_masuk,
						LEFT(jam_pulang, 5) AS jam_pulang,
						LEFT(jam_batas_pulang, 5) AS jam_batas_pulang,
						LEFT(jam_batas_masuk, 5) AS jam_batas_masuk
		 			FROM m_set_waktu WHERE 1=1 ".$where." ".$order_by." ".$sql_limit."";
	$data_list = $db->select($sql_tampil);

	$sql_total = "SELECT id FROM m_set_waktu WHERE 1=1 ".$where." ";
...
if(isset($_GET['hapus'])) {
	$id = $_POST['id'];
	$hapus = $db->delete('m_set_waktu', $id);
	if($hapus) {
		echo 'OK';
	}
	exit();
		...
		<div class="box box-primary">
			...
			<div class="box-body">
				...
				<div id="toolbar" class="">
					<a href="<?php echo site_url('?m=waktu&c=tambah'); ?>" class="btn btn-flat btn-success">
				...
				<table 
					...
					data-toggle="table"
					data-id-field="id"
					data-url="<?php echo site_url('?m=waktu&ajax=1'); ?>" 
					data-sort-name="nama_seting"
					data-sort-order="asc"
					...
					<thead>
						<tr>
							...
							<th data-field="nama_seting" data-sortable="true" data-valign="middle">Nama Seting</th>
							<th data-field="jam_mulai_abs" data-sortable="true" data-valign="middle">Jam Mulai Absen</th>
							<th data-field="jam_masuk" data-sortable="true" data-valign="middle">Jam Masuk</th>
							<th data-field="jam_batas_masuk" data-sortable="true" data-valign="middle">Jam Batas Masuk</th>
							<th data-field="jam_pulang" data-sortable="true" data-valign="middle">Jam Pulang</th>
							<th data-field="jam_batas_pulang" data-sortable="true" data-valign="middle">Jam Batas Pulang</th>
...
<script type="text/javascript">
	...
	$(function() {
		...
		$('#link_konfirmasi_hapus').click(function(event) {
			...
			//var id_hapus = 0;
			$.ajax({
				url: '<?php echo site_url('?m=waktu&hapus=1'); ?>',
				type: 'POST',
				dataType: 'html',
				data: {id: id_hapus},
			})
	...
	function aksi_ft(value, row, index) {
		var nsi_out = '<a class="btn btn-flat btn-sm btn-primary" href="<?php echo site_url('?m=waktu&c=ubah&id'); ?>=' + row.id + '" title="Ubah"><i class="fa fa-pencil"></i></a>';
		if(row.id >= 1) {
